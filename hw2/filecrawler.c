...
static void HandleDir(char *dirpath, DIR *d, DocTable *doctable,
  ...
  while (1) {
    ...
    if (dirent == NULL) return;  // end of dir
    ...
    if (res == 0) {
      ...
      if (S_ISREG(nextstat.st_mode))
        HandleFile(newfile, doctable, index);  // regular file
      if (S_ISDIR(nextstat.st_mode)) {  // directory
        DIR* subdir = opendir(newfile);
        if (subdir == NULL) {
          free(newfile);
          continue;  // failed open
        }
        HandleDir(newfile, subdir, doctable, index);
        closedir(subdir);
      }
    }
...
static void HandleFile(char *fpath, DocTable *doctable, MemIndex *index) {
  ...
  while (NumElementsInHashTable(tab) > 0) {
    ...
    // inverted index.
    int res = HTIteratorDelete(it, &kv);
    Verify333(res != 0);
    wp = kv.value;

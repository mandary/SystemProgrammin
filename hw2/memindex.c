...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    Verify333(wds != NULL);
    wds->word = word;
    wds->docIDs = AllocateHashTable(1);
    Verify333(wds->docIDs != NULL);
    kv.key = wordkey;
    kv.value = wds;
    res = InsertHashTable(index, kv, &hitkv);  // insert to index
    Verify333(res == 1);
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  wds = kv.value;
  HTIter it = HashTableMakeIterator(wds->docIDs);  // iterate through all docs
  Verify333(it != NULL);
  SearchResultPtr searchResult;

  while (!HTIteratorPastEnd(it)) {
    res = HTIteratorGet(it, &kv);
    Verify333(res == 1);
    searchResult = (SearchResultPtr)malloc(sizeof(SearchResult));
    Verify333(searchResult != NULL);
    searchResult->docid = kv.key;
    searchResult->rank = NumElementsInLinkedList((LinkedList)kv.value);
    AppendLinkedList(retlist, (LLPayload_t)searchResult);
    HTIteratorNext(it);
  }
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      LLIteratorGetPayload(llit, (LLPayload_t)&searchResult);
      wordkey = searchResult->docid;
      res = LookupHashTable(wds->docIDs, wordkey, &kv);
      if (res == 0) {  // not in new word's docIDs
        LLIteratorDelete(llit, free);
      } else {  // in new word's docIDs
        searchResult->rank += NumElementsInLinkedList((LinkedList)kv.value);
        LLIteratorNext(llit);
      }
    }

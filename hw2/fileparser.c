...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  while (left_to_read > 0) {
    ...
    if (result == 0) {  // EOF
      left_to_read = 0;
    } else if (result == -1) {  // error reading
      if (errno == EAGAIN || errno == EINTR) {
        free(buf);
        return NULL;
      }
    }
    numread += result;  // update pointer
    left_to_read -= result;
  }
...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  int i = 0;  // current pos
  int pos = 0;  // for wordstart pos
  while (*curptr) {  // if not '\0'
    while (*curptr && !isalpha(*curptr)) {  // not a word
      *curptr = '\0';
      i++;
      curptr++;
    }
    ...
    pos = i;
    wordstart = curptr;
    while (*curptr && isalpha(*curptr)) {
      *curptr = tolower(*curptr);  // to lower case
      i++;
      curptr++;
    }
    if (!*curptr) {  // if not '\0'
    ...
    *curptr = '\0';  // set it as end of word
    AddToHashtable(tab, wordstart, pos);
    curptr++;
    i++;
  }
...
static void AddToHashtable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    strncpy(newstr, word, strlen(word) + 1);  // copy the string
    wp->word = newstr;
    ...
    HTKeyValue old;
    retval = InsertHashTable(tab, kv, &old);  // insert into hashtable
    Verify333(retval == 1);
  }
}

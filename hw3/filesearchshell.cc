...
int main(int argc, char **argv) {
  ...
  hw3::QueryProcessor prossor(files, true);
  while (1) {
    std::cout << "Enter query:" << std::endl;
    std::string word, input;
    getline(std::cin, input);  // get query line
    transform(input.begin(), input.end(), input.begin(), ::tolower);  // to lower case
    std::stringstream ss(input, std::stringstream::in);

    std::vector<std::string> query;  // query
    ...
    std::vector<hw3::QueryProcessor::QueryResult> result = prossor.ProcessQuery(query);
    PrintResult(result);
  }
  return EXIT_SUCCESS;
}
...
// This prints out the result from result
static void PrintResult(std::vector<hw3::QueryProcessor::QueryResult> result) {
      // if found
    if (result.size() > 0) {

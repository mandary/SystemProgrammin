...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    LLIteratorGetPayload(it, reinterpret_cast<void**>(&position));
  ...
  return sizeof(header) + num_pos_ho * sizeof(docid_element_position);
}
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  uint16_t wordlen_ho;
  // MISSING:
  wordlen_ho = strlen(reinterpret_cast<char*>(wds->word));
...
static HWSize_t WriteHeader(FILE *f,
  ...
  if (fwrite(&header, sizeof(header), 1, f) != 1) return 0;
...
static HWSize_t WriteBucketRecord(FILE *f,
  ...
  br = {NumElementsInLinkedList(li), b_offset};
...
static HWSize_t WriteBucket(FILE *f,
  ...
  // "fn" parameter to invoke the helper function that knows how total
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      element_position_rec header = {nextelpos};
      header.toDiskFormat();


      res = fwrite(&header, sizeof(element_position_rec), 1, f);
      if (res != 1) {
        LLIteratorFree(it);
        return 0;
      }
      ...
      // Write element
      offset += sizeof(element_position_rec);
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    if (NumElementsInLinkedList(li) != 0) {
